...
def test_lcm_discover():
    lcm = LCM(min_supp=3)
    patterns = lcm.fit_transform(D)  # get new pattern set
...
def test_lcm_max():
    ...
    patterns = lcm.fit_transform(D, return_tids=True)
...
def test_lcm_lexicographic_order():
    ...
    patterns = lcm.fit_transform(D, lexicographic_order=True)
    longest_itemset = max(patterns.itemset, key=len)
    assert longest_itemset == [1, 4, 6]
...
def test_lcm_file_output():
    lcm = LCM(min_supp=3)
    lcm.fit_transform(D, lexicographic_order=True, out="./skmine/itemsets/tests/lcm_out.dat")
...
def test_max_length_lcm():
    ...
    patterns = lcm.fit_transform(D, lexicographic_order=True, max_length=2)
    longest_itemset = max(patterns['itemset'], key=len)
    assert longest_itemset != [1, 4, 6]
