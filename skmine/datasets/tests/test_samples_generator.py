...
def test_make_transactions():
    ...
    D = make_transactions(n_transactions, n_items, 0.5)
...
def test_make_transactions_high_density():
    D = make_transactions(100, 10, 0.99)
...
def test_make_transactions_bilateral():
    ...
    """
    properties = dict(n_transactions=150, n_items=30, density=0.5)
...
def test_make_transactions_random_state():
    ...
    properties = dict(n_transactions=150, n_items=30, density=0.2, random_state=2)
    D1 = make_transactions(**properties)
    D2 = make_transactions(**properties)
    desc1 = describe(D1)
    desc2 = describe(D2)
    assert desc1 == pytest.approx(desc2, abs=0.1)
...
def test_make_classification():
    ...
    full_desc = dict(
        ...
        n_transactions=100,
        density=0.40,  # both have .5 densities, but only 20% of it is intersecting, see class_sep
    )
