...
def compress_size(D, patt, seq_lengths):
    ...
    D = {k: D[k].copy() for k in patt}  # check in pattern, other events are left untouched anyway
    patt = tuple(patt)
    ...
    for start_pos, seq_end_idx in zip(start_positions, seq_end_indices):
        ...
        for e in patt[1:]:
            ...
            if pos > seq_end_idx:
                break
            if bits and pos <= bits.max():  # avoid error
                next_set_bit = bits.next_set_bit(pos)
                indices.append(next_set_bit)
                pos = next_set_bit
        if len(indices) == len(patt):
...
def best_compressing(D, seq_lengths):
    """
    ...
    while len(D) - len(p) > 0:
        ...
        e = max(cands, key=lambda k: (len(D[k]), estimate_gain(D[k], seq_lengths)))  # TODO : lazy estimate gain
        p.append(e)
        comp_size, u_d = compress_size(D, p, seq_lengths)
        if comp_size < b:
            b = comp_size
            update_d = u_d
        else:
...
def estimate_gain(tids, seq_lengths):
    ...
    for i in range(len(seq_lengths) - 1):
        ...
        _tids = list(takewhile(lambda e: e < end, tids.iter_equal_or_larger(start)))
        diff = np.diff(_tids)
...
class GoKrimp(BaseMiner, MDLOptimizer):  # TODO : inerit MDL Optimizer
    ...
    def fit(self, D):
        ...
        self.standard_codetable_ = {event : tids.copy() for event, tids in vert_D.items()}
