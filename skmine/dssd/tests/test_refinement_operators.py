...
# write tests for the offical improved version of the operator


def test_generate_numeric_conditions(): 
    conds = [
        Cond("a", "<", 2.0), Cond("a", ">", 2.0),
        Cond("a", "<", 4.0), Cond("a", ">", 4.0),
        Cond("a", "<", 6.0), Cond("a", ">", 6.0)
    ]
    assert ro.generate_numeric_conditions("a", 0, 8, 3, ["<", ">"]) == conds


def test_basic_generate_conditions():
    numeric_operators = ["<", ">"]
    conds_store = [
        Cond("num", "<", 2.0), Cond("num", ">", 2.0), Cond("num", "<", 4.0), Cond("num", ">", 4.0),
        Cond("a", "==", "one"), Cond("a", "!=", "one"), Cond("a", "==", "two"), Cond("a", "!=", "two"),
        Cond("bin", "==", True), Cond("bin", "==", False), 
    ]

    gened_conditions = ro.generate_conditions(df, column_types.keys(), column_types, discretization, numeric_operators)
    assert set(gened_conditions) == set(conds_store)


def test_basic_refinements():
    numeric_conditions = [Cond("num", "<", 2.0), Cond("num", ">", 2.0), Cond("num", "<", 4.0), Cond("num", ">", 4.0)]
    nominal_conditions = [Cond("a", "==", "one"), Cond("a", "!=", "one"), Cond("a", "==", "two"), Cond("a", "!=", "two")]
    binary_conditions = [Cond("bin", "==", True), Cond("bin", "==", False)]
    conds_store = [*numeric_conditions, *nominal_conditions, *binary_conditions]
    basic_ro: ro.RefinementOperatorBasic = ro.create("basic", extra_parameters)

    # testing the append method
    new_cand1 = Subgroup(Description([Cond("", "<", "")]), cover=pandas.Index(range(3)), parent=parent_candiate)
    assert basic_ro.check_and_append_candidate(new_cand1, []) == [new_cand1]
    assert basic_ro.check_and_append_candidate(Subgroup(Description(), parent=parent_candiate), []) == []

    # update the 
    start_cand = Subgroup(Description())
    start_cand.cover = mock_cover(start_cand).index

    cands_step1 = basic_ro.refine_candidate(start_cand, [])
    ...
    # update the start cand and begin a new refinement operation
    start_cand = cands_step1[0]
    cands_step2 = basic_ro.refine_candidate(start_cand, [])
